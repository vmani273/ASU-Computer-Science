...
void initializeStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	for(int x = 0; x < NUM_STRINGS * STRING_LENGTH; x++) { // for loop that goes through program and sets all the characters in each string to zero
		*(ptr+1) = '\0';
	}
...
{
	char *ptr = &strings[0][0];
    for(int x = 0;x < NUM_STRINGS; x++) { // for loop that utilizes pointers to print out each value 
		ptr= &strings[x];
		printf("%s\n",ptr);
	}
}
...
char* reverseOneString(char s[STRING_LENGTH])
{
	...
	// enter code here
	int lastValue = strlen(s) - 1;

	for(int x = 0; x < strlen(s) / 2; x++) // for loop to iterate through loop
	{
		temp = *(p + x); // set temporary variable to last value, or corresponding value 
		*(p + x) = *(p + lastValue); // switch first value and last value 
		*(p + lastValue) = temp; // set last value to temp
		lastValue--;
	}
...
{
	char *ptr = &strings[0][0];
	for(int x = 0; x < NUM_STRINGS; x++) { // iterating through the array and using "reverseOneString" method to print out series of strings
		reverseOneString(strings[x]);
	}
...
void encryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	char *p = &strings[0][0];
	reverseStrings(p);
	for(int x = 0; x < NUM_STRINGS * STRING_LENGTH; x++) { // using for loop to iterate through the array
		if (*(p + x) != '\0') {
			*(p + x) += key; // if the value is not "\0", add by key digits
		}
...
void decryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	for(int x = 0; x < NUM_STRINGS * STRING_LENGTH; x++) { // using for loop to iterate through the array
		if (*(p + x) != '\0') {
			*(p + x) -= key; // if the value is not "\0", subtract by key digits
		}
...
int isPalindrome(char s[STRING_LENGTH])
{
	...
	for (backwards = s; p >= backwards; backwards++) {
		if (*p == *backwards) { // checking order of reverese and if it is the most suitable
			--p; // subtract p by one 
		}
		else {
			break;
	...
	if (backwards < p) // either one corrosponds to one or zero
		palindrome = 0; // set palindrome variable to 0
	else
		palindrome = 1; // set palindrome variable to 1
